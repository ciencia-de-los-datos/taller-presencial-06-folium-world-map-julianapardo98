...
def add_countries_column(affiliations):
    ...
    affiliations["countries"] = affiliations["countries"].map(set) #Para que no se repitan paises. Hago conjunto la lista de paises
    affiliations["countries"] = affiliations["countries"].str.join(", ")

    return affiliations

def clean_countries(affiliations):
    #Se hace porque a veces articulos viejos con nombres de paises antes de cambiarlos, entonces no se reconocen
    affiliations = affiliations.copy()
...
def count_country_frequency(affiliations):
    ...
    countries = countries.str.split(", ") #Pongo los paises separados en lista
    countries = countries.explode() #Me saca los paises 
    countries = countries.value_counts() #Cuento cada regsitro y da la cantidad de veces
    return countries
...
def plot_world_map(countries):
    ...
    countries = countries.to_frame() #Solo columnas y # de veces lo vuelve data frame
    countries = countries.reset_index() #Resetea el index para que no sean los paises sino 0,1,2...
